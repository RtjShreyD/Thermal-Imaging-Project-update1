...
function [location,p]=new_right_points2(a,crotch,hiplevel)
[X,Y,x,y]=mysmooth(a);
...
try
    location(5)=find(abs(y(location(3):location(4))-hiplevel)<0.001)+location(3)-1;    %%location3 is added so that it comes in the blob indexing
    %location(5)=find(abs(maxfun2(y)-hiplevel))+location(4)-1;   %%not correct
    % location(5) = find(abs(hiplevel))+location(4)-1; %This is alternative
    % method for Hip points 
    p(5,1)=X(location(5));
    p(5,2)=Y(location(5));
catch
    fprintf('error in newrightpooints\n');
    [~,hipright] = newhipfront(a);
    p(5,1)=x(hipright);
    p(5,2)=y(hipright);
end
%%%Hip in new right pts was giving error for some images tried to resolve
%%%that issue from line 104 to 110, and commenting lines 93,95,96 for the 
%%% resolving method stated here is to be used only for images for which
%%% new right points give error. Even resolved method is not accurate at
%%% for some use it only when either original gives error or accuracy is ver y less in lod algorithm.
