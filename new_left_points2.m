function [location,p]=new_left_points2(a,cro,hiplevel)
[X,Y,x,y]=mysmooth(a);
...
% % % hip
try
    location(5)=find(abs(y(location(4):location(3))-hiplevel)<0.001)+location(3)-1;
% location(5)=find(abs(hiplevel))+location(4)-1; %This is alternative
% method for Hip points 
    p(5,1)=X(location(5));
    p(5,2)=Y(location(5));
catch
    fprintf('error in newleftpooints\n');
    [hipleft,~] = newhipfront(a);
    p(5,1)=x(hipleft);
    p(5,2)=y(hipleft);
end

%%%Hip in new right pts was giving error for some images tried to resolve
%%%that issue from line 77 to 83, and commenting lines 67,68,69 for the
...
%%% for some use it only when either original gives error or accuracy is very less in old algorithm.
